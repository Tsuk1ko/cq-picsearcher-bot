...
```javascript
{
    ...
    "picfinder": {
        ...
        "proxy": "",               // 大部分请求所使用的代理，支持 http(s):// 和 socks://
        // 复读机
        "repeat": {
            "enable": true,        // 开关
        ...
        "reminder": {
            "enable": false,    // true 则启用
            "onlyPM": false,    // 仅私聊使用
            "onlyAdmin": false  // 仅管理者(picfinder.admin)使用
        }
    },
    // 数据库配置（用于缓存搜图结果）
    "mysql": {
...
如果你在配置中选择使用 sqlite，那么你完全不需要自行配置或部署 mysql
...
> 该功能为测试功能，未设计完善的限制机制，请暂时不要开放给他人使用

该功能用于创建高度自定义的定时文字提醒功能，可在私聊、群组、讨论组中创建，每个区域的上限各为 20 条

该功能默认关闭，需要配置`config.json`启用（请看很前面的配置说明）

功能启用时，无论`onlyPM`如何设置，管理者(`picfinder.admin`)一定可以在群内使用

#### 创建

无需@，也不要@机器人，直接发送`--rmd=提醒内容 --time=时间表达式`，这两个参数的先后顺序没有关系，提醒内容如果含有空格和换行也无所谓，该怎么输入就怎么输入
...
注意：为了防止刷屏现象，不允许使用秒级 cron 表达式，并且**分钟**级的最小分度为`5`，即最低只能`*/5 * * * *`
...
查看提醒列表时可以看到提醒对应的 ID，然后直接发送`--rmd-del=ID`即可，只能删除当前区域创建的提醒
