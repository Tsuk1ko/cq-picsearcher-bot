...
该功能用于创建高度自定义的定时文字提醒功能，可在私聊、群组、讨论组中创建，上限为 20 条

该功能默认关闭，需要配置`config.json`启用（请看很前面的配置说明）

#### 创建

无需@，也不要@机器人，直接发送`--rmd=提醒内容 --time=时间表达式`，提醒内容如果含有空格和换行也无所谓，该怎么输入就怎么输入

如果提醒需要@人（@自己也是），请在提醒内容中直接附上@（必须是真实的@，纯文字@是无效的）

时间表达式的格式与 crontab 相同，如下所示，**只不过需要将空格换成英文逗号`,`**，即格式应为`a,b,c,d,e`
...
crontab 表达式的每一位都可以配合`/`或者`-`符号实现每隔一定单位时间或某个区间时间提醒的功能，不单单只能固定时间提醒，详情请自行百度
